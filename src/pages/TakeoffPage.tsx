...
import { appendPdfProvenance, createA4LandscapePdfBlobFromCanvases, createPdfBlobFromCanvas, createPdfProvenancePages, openExportBlob, openExportTab, warmPdfExportModule } from "../export/pdf";
...
async function exportChartPdf(inputs: TakeoffInputs, result: TakeoffResult, exportContext: ExportContext, options: { openWindow?: Window | null } = {}) {
  const { canvas, exportDate } = await createTakeoffExportCanvas(inputs, result, exportContext);
  const pages = createPdfProvenancePages(canvas, exportContext, CHART_PAGE_SPLIT_Y);
  const blob = await createA4LandscapePdfBlobFromCanvases(pages);
...
async function createTakeoffPathExportCanvas(inputs: TakeoffInputs, result: TakeoffResult, exportContext: ExportContext, aircraftLabel: string) {
  ...
  drawWrappedExportText(context, exportContext.warnings.length ? `Warnungen: ${exportContext.warnings.map((warning) => warning.text).join(" · ")}` : "Warnungen: keine.", 48, rowY + 14, 1070, { color: exportContext.warnings.length ? "#9a5200" : "#526274" });
...
async function createTakeoffExportCanvas(inputs: TakeoffInputs, result: TakeoffResult, exportContext: ExportContext) {
  ...
  drawExportText(context, exportContext.warnings.length ? `Warnungen: ${exportContext.warnings.map((warning) => warning.text).join(" · ")}` : "Warnungen: keine", 48, 718, { size: 18, color: exportContext.warnings.length ? "#9a5200" : "#526274" });
